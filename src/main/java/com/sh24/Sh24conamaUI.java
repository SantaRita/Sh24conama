...
import com.csi_ti.itaca.custom.general.frontend.ProvPantallaBusquedaDocumentos;
import com.csi_ti.itaca.custom.general.frontend.ProvPantallaEjecutarConsulta;
...
@Title("Consultas AMA" )
@SpringUI
public class Sh24conamaUI extends UI {
	...
	Label lblProveedores = new Label("WEB de Consultas AMA");
...
private void doLogin() {
		...
		/*if (  !usuario.getValue().toUpperCase().equals("AMA_ADMON") )  {
			new Notification("Error de usuario/password",
					"No tiene acceso a esta aplicación.<br>",
		...
		try {
			...
			if (conexionOK)  {
					...
					if ( !retLogin.get("CODIGOERROR").toString().equals("0")) {
						...
						UI.getCurrent().getNavigator().addView("ProvPantallaEjecutarConsulta", ProvPantallaEjecutarConsulta.class);
						UI.getCurrent().getNavigator().addView("ProvPantallaBusquedaExpedientes", ProvPantallaBusquedaDocumentos.class);
